...
def detect_language(text: str) -> Dict[str, str]:
    ...
    Returns:
        Dict containing:
            ...
            - is_multilingual: "true" or "false"
            
    Raises:
        ValueError: If API request fails or response is invalid
    """
    prompt = (
        f"Detect the language of this text and respond ONLY with valid JSON "
        f'in this exact format: {{"source_language": "<language>", "is_multilingual": true|false}}\n\n'
        f"Text: {text}"
    )
    
    response = _call_qubrid_api(prompt, temperature=0.1)
    ...
    try:
        ...
        return {
            ...
            "is_multilingual": str(result.get("is_multilingual", False)).lower()
        }
...
def translate_text(text: str, target_language: str) -> Dict[str, str]:
    ...
    """
    prompt = (
        f"Translate this text to {target_language} and respond ONLY with valid JSON "
        f'in this exact format: {{"translated_text": "<translation>", "target_language": "{target_language}"}}\n\n'
        f"Text: {text}"
    )
    
    response = _call_qubrid_api(prompt, temperature=0.7)
    
    try:
        # Try to parse JSON from response
        result = json.loads(response)
        return {
            "translated_text": result.get("translated_text", response),
